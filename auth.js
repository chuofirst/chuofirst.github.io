...
// すべての子要素を隠す
Array.from(document.body.children).forEach(child => {
if (child.id !== ‘request-screen’ && child.id !== ‘login-screen’) {
...
loginDiv.innerHTML = `<div style="width: 400px; height: 100px; margin-bottom: 40px; display: flex; align-items: center; justify-content: center;"> <div id="logo-placeholder-login" style="color: white; font-size: 1.5em; font-weight: bold;">中央中等生ファーストの会</div> </div> <div style="background: white; padding: 40px; border-radius: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.3); max-width: 500px;"> <h2 style="margin: 0 0 20px 0; color: #333;">ログイン</h2> <p style="margin: 0 0 30px 0; color: #666;">承認済みのメールアドレスでログインしてください。</p> <input type="email" id="login-email-input" placeholder="メールアドレス" style=" width: 100%; padding: 15px; font-size: 1em; border: 2px solid #ddd; border-radius: 10px; margin-bottom: 20px; box-sizing: border-box; "> <button id="login-btn" style=" width: 100%; background: #ee7800; color: white; font-size: 1.2em; font-weight: 700; padding: 15px; border: none; border-radius: 10px; cursor: pointer; margin-bottom: 15px; transition: all 0.3s ease; " onmouseover="this.style.background='#ff9933'" onmouseout="this.style.background='#ee7800'"> ログイン </button> <button id="back-to-request-btn" style=" width: 100%; background: #ccc; color: #666; font-size: 1em; padding: 10px; border: none; border-radius: 10px; cursor: pointer; "> まだ申請していない方はこちら </button> <div id="login-status" style="margin-top: 20px; text-align: center; color: #999; font-size: 0.9em;"></div> </div>`;
