...
function checkMemberThreadAccess() {
...
function waitAuthAndLoad() {
  const timer = setInterval(() => {
    authCheckCount++;
    const ok = checkMemberThreadAccess();
...
function appendPost(index, timestamp, name, body, imageUrls) {
  ...
  if (imageUrls && imageUrls.trim() !== "") {
    ...
    urls.forEach(raw => {
      ...
      console.log("[MEMBER_THREAD] raw image url from sheet:", url);
      ...
      console.log("[MEMBER_THREAD] img src used:", displayUrl);
...
const SHEET_URL = "https://docs.google.com/spreadsheets/d/1wAzJwnhOBDtE8a8spfvrNGQr0QNh-lhA044Pmq1mvGY/gviz/tq?sheet=Responses";
...
async function loadPosts() {
  ...
  if (!SHEET_URL || SHEET_URL.includes("[ここに")) {
    threadBody.innerHTML = '<div class="blog-loading">スプレッドシートURLが設定されていません。<br>member_thread_sheet.jsのSHEET_URLを設定してください。</div>';
    return;
  }
  ...
  try {
    ...
    rows.forEach((row, idx) => {
      ...
      console.log("[MEMBER_THREAD] row", idx + 1, "imageUrls =", imageUrls);
